...
UI.Outliner.prototype.setOptions = function ( options ) {
	...
	window.moveObject = function( object, newParent, nextObject ) {
        ...
        if (nextObject === undefined) {
            if (newParent instanceof THREE.Scene) {

            } else if (object.parent instanceof THREE.Scene) {
                //object.position.set(
                //    object.position.x - newParent.position.x,
                //    object.position.y - newParent.position.y,
                //    object.position.z - newParent.position.z);
            }
        }
...
UI.THREE.Boolean = function ( boolean, text ) {
    ...
    var _guid = THREE.Math.generateUUID();
    this.checkbox = new UI.Checkbox(boolean);
    this.checkbox.dom.setAttribute("id", _guid);
    this.text = new UI.Label(text, _guid).setMarginLeft('3px');
